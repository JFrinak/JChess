...
public class Table extends Observable{
	...
	private static String defualtPieceImagesPath = "art/simple/";
	...
	private class TilePanel extends JPanel {
		...
		private void assignTilePieceIcon(final Board board){
			...
			if(board.getTile(this.tileId).isTileOccupied()){
				try {
					final BufferedImage image = ImageIO.read(new File(defualtPieceImagesPath +
		...
		private Collection<Move> pieceLegalMoves(final Board board) {
			if(humanMovedPiece != null && humanMovedPiece.getPieceAlliance() == board.currentPlayer().getAlliance()){
				List<Move> thePieceLegalMoves = new ArrayList<>(humanMovedPiece.calculateLegalMoves(board));
				if(humanMovedPiece.getPieceType().isKing()){
					Collection<Move> castlingMoves = new ArrayList<>();
					for(final Move move: board.currentPlayer().getLegalMoves()){
						if(move.isCastlingMove()){
							castlingMoves.add(move);
						}
					}
					thePieceLegalMoves.addAll(castlingMoves);
				}
				return thePieceLegalMoves;
			}
